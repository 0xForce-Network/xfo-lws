...
This review is meant to serve as an informal audit of `xfo-lws` as of
February 3, 2022. It is divided into 3 parts:

1. How does `xfo-lws` work?
2. The code
3. Suggestions

It was put together as part of [a CCS proposal](https://ccs.getmonero.org/proposals/j-berman-3-months-full-time.html)
with the intention of getting `xfo-lws` further along (and potentially 
...
`xfo-lws`, and has to-do's for a number of folders and files. 

In my review, I did not find anything that would compromise `xfo-lws` when
examining from the lens of the following threat model:

```
A user who is running `monerod` + `xfo-lws` on a machine only the user has 
...
- a backdoor that sends sensitive data from `xfo-lws` out to a 3rd party
...
curious users, potential contributors, or any future auditors of `xfo-lws`.
...
## How does `xfo-lws` work?

Monero wallets need to scan all transactions in the blockchain to find which 
ones belong to a user. `xfo-lws` ("Monero light wallet server") is designed 
...
running `xfo-lws` instance, so that when the user revisits the app, the app
is immediately ready to use for spending. `xfo-lws` works by taking in a 
user's address and view key, and perpetually scanning for transactions received
and plausibly spent by the user.

The primary risk to a user using `xfo-lws` as their backend is that the
...
`xfo-lws` backend is hosted will be able to determine all outputs received to
the user, and in most cases today, the outputs spent by the user as well. This
also includes **amounts** received to and spent by the user. If a `xfo-lws` 
instance is compromised, user funds are still SAFU since the private spend key
is never sent to `xfo-lws`. Ongoing research may improve this risk profile
further, but the above should be users' default assumptions today.

Whoever hosts a `xfo-lws` instance can run the server via the
`xfo-lws-daemon` as described in the [README.md](/README.md#running-xfo-lws-daemon)
("Running xfo-lws-daemon"). The daemon is expected to run 24/7 and
perpetually scan for transactions recevied to (and plausibly spent from) the
addresses known to the server in real-time.

Whoever hosts a `xfo-lws` instance can perform administrative tasks via the
`xfo-lws-admin` program, as described in
[administration.md](/docs/administration.md). The `xfo-lws-daemon` does not
need to be running in order for the admin to execute commands via
`xfo-lws-admin`.

Both `xfo-lws-daemon` and `xfo-lws-admin` read/write from/to an LMDB
database on the same machine hosting `xfo-lws`.

`xfo-lws-daemon` can also fetch exchange rates from a 3rd party API provider
if an administrator wants. This is turned off by default.

Here is a simplified diagram of the `xfo-lws` architecture:
...
potentially improve `xfo-lws`.

## The code

### /src

This is where the source code lives. The starter file for `xfo-lws-daemon` is
[/src/server_main.cpp](/src/server_main.cpp). The starter file for
`xfo-lws-admin` is [/src/admin_main.cpp](/src/admin_main.cpp). Both use
...
Passing `--help` to `xfo-lws-daemon` is the one exception that does not run
...
initializes all tables in the db, syncs `xfo-lws` with the running `monerod` 
...
is an arg the admin can set when starting the `xfo-lws-daemon`. This caching
...
a status the admin can set via `xfo-lws-admin`. If hidden, it will return
...
`xfo-lws-admin` in order to add the address to the server.
...
higher frequency. Thus, it is ok that `xfo-lws` sets this `recent_cutoff` 
...
issue](https://github.com/vtnerd/xfo-lws/issues/25) for more.
...
`xfo-lws-daemon` is running. It makes sure `xfo-lws` is in sync with the
...
Sets the `xfo-lws-daemon`'s `running` state to false. It is called from
`server_main.cpp`, which is listening for a SIGINT signal in order to trigger
`lws::scanner::stop()`.

##### lws::scanner::sync()

Syncs the running `xfo-lws-daemon` with the blockchain, making sure 
`xfo-lws-daemon` knows what blocks still need to be scanned.

First thing this function does is reads the `xfo-lws` db to find the most
...
`xfo-lws` db, and rolls back/updates the chain if a reorg has been detected.
...
transactions while `xfo-lws-daemon` is running. Also updates exchange rates
held in the cache.

First retrieives all active accounts and their received outputs from the 
`xfo-lws` db. If no active accounts, sleeps for a bit. Otherwise, proceeds
...
initialized. Then calls `lws::scanner::sync()` to make sure `xfo-lws` is
...
the users stored in the `xfo-lws` db. If it finds that there is a change
...
- Share a guide on hosting `xfo-lws` behind a .onion, and enable a frontend
to make calls to it.

- The exchange rate API defaults to off. When turned on, it fetches prices in a
custom interval over the clearnet. `xfo-lws` could be bundled with tor or
