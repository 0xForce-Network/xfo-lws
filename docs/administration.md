# xfo-lws Administration
The `xfo-lws-admin` executable or `--admin-rest-server` option in the
`xfo-lws-daemon` executable can be used to administer the database
used by `xfo-lws-daemon`. Any number of `xfo-lws-admin` instances can run
concurrently with a single `xfo-lws-daemon` instance on the same database.
...
The `xfo-lws-admin` utility is structured around command-line arguments with
...
`xfo-lws-admin` in a shell environment. The `jq` program can be used for
indenting the output to make it more readable, and can be used to
search+filter the JSON output from the command.

# Admin REST API
The `xfo-lws-daemon` can be started with 1+ `--admin-rest-server` parameters
...
An admin account account can be created via `xfo-lws-admin create_admin`
...
Running `xfo-lws-admin list_admin` will display all current admin
...
starting `xfo-lws-daemon`. The webhook will fail if there is a mismatch of
...
If you use the `debug_database` command provided by the `xfo-lws-admin`
...
**List every active Monero address on a newline:**
  ```bash
  xfo-lws-admin list_accounts | jq -r '.active | .[] | .address'
  ```

**Auto-accept every pending account creation request:**
  ```bash
  xfo-lws-admin accept_requests create $(xfo-lws-admin list_requests | jq -j '.create? | .[]? | .address?+" "')
  ```
# Debugging

`xfo-lws-admin` has a debug mode that dumps everything stored in the
...
**List every key-image associated with every account:**
  ```bash
  xfo-lws-admin debug_database | jq '."spends_by_account_id,block_id,tx_hash,output_id" | map_values([.[] | .image])'
  ```
...
**List every account that received XMR in a given transaction hash:**
  ```bash
  xfo-lws-admin debug_database | jq '."outputs_by_account_id,block_id,tx_hash,output_id" | map_values(select([.[] | .tx_hash == "hash"] | any)) | keys'
  ```
...
**Add total received XMR for every account**:
  ```bash
  xfo-lws-admin debug_database | jq '."outputs_by_account_id,block_id,tx_hash,output_id" | map_values([.[] | .amount] | add)'
  ```
will output something like:
  ```json
  {"1":6346,"2":45646}
...
# Extending Administration in xfo-lws
